...
def add_book(library):
    ...
    read = input('Enter you read the book? (yes/no): ').lower() == 'yes'
...
def search_library(library):
    ...
    if results:
        for book in results:
            status = "Red" if book['read'] else "unread"
            print(f"{book['title']} by {book['author']} - {book['year']} - {book['genre']} - {status}")
    else:
         print(f"No books found matching '{search-term}' in the {search-by} field. ")
